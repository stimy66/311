...
let restaurantPhone = "213XXXXXXXXX"; // <-- عدّل إلى رقمك الفعلي بصيغة دولية
...
function renderCart() {
  ...
  if (cart.length === 0) {
    cartItemsEl.innerHTML = '<p class="text-gray-400">لا توجد عناصر في السلة بعد. أضف بعض الأطباق اللذيذة!</p>';
    calculateTotals();
    saveCart();
    return;
  }
